...
struct DriveStudioSlotWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName(Text(verbatim: "Drive Studio Slot \(slotIndex + 1)"))
        .description(Text(verbatim: "Renders the widget assigned to Slot \(slotIndex + 1)."))
...
struct DriveStudioWidgetEntryView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                let mockupSpec = (entry.isPreview && entry.slot == nil) ? ApexMockups.spec(for: entry.slotIndex) : nil
                let activeSpec = mockupSpec ?? entry.slot?.spec
                ...
                } else if let spec = activeSpec, let layers = spec.layers, !layers.isEmpty {
                    FitToCanvasLayers(
                        layers: layers,
                        canvasSize: geo.size,
                        telemetry: entry.telemetry,
