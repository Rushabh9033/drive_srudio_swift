...
struct DriveEntry: TimelineEntry {
    let date: Date
    var speed: Double       // Real live vehicle speed
    var batteryLevel: Double // Real UIDevice battery level (0.0 to 1.0)
    var isCharging: Bool
    var vehicleName: String  // Selected vehicle profile name
}
...
struct CommandCenterView: View {
    ...
    var body: some View {
        ...
        let batteryPct = max(0.05, min(1.0, e.batteryLevel))
...
struct VortexDriveView: View {
    ...
    var body: some View {
        ...
        let speedFrac = max(0.01, min(1.0, e.speed / 200.0))
        ...
        let content = ZStack {
            ...
            GeometryReader { g in
                ...
                ZStack {
                    ...
                    Circle().trim(from: 0, to: CGFloat(speedFrac))
                    ...
                    VStack(spacing: 2) {
                        ...
                        Text("\(Int(e.speed)) km/h").font(.system(size: s*0.07, weight: .bold)).foregroundColor(Color(hex: "F97316"))
                    }.position(x: cx, y: cy)
                }
            }
        }.clipShape(RoundedRectangle(cornerRadius: 22, style: .continuous))
            ...
            VStack(spacing: 6) {
                HStack(spacing: 6) {
                    ...
                    ZStack {
                        ...
                        VStack(spacing: 2) {
                            Text("\(Int(e.speed))").font(.system(size: 26, weight: .black, design: .rounded)).foregroundColor(Color(hex: "00F0FF"))
                    ...
                    ZStack {
                        RoundedRectangle(cornerRadius: 12).fill(Color(hex: "0D1A1A"))
                        VStack(spacing: 4) {
                            Text("\(Int(batteryPct*100))%").font(.system(size: 20, weight: .black, design: .rounded)).foregroundColor(Color(hex: "22C55E"))
                            GeometryReader { g in
                                ZStack(alignment: .leading) {
                                    RoundedRectangle(cornerRadius: 4).fill(Color(hex: "1A3A1A")).frame(height: 8)
                                    RoundedRectangle(cornerRadius: 4).fill(Color(hex: "22C55E")).frame(width: g.size.width*CGFloat(batteryPct), height: 8)
                                }
            ...
            GeometryReader { g in
                ...
                ZStack {
                    ...
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 3).fill(Color(hex: "22C55E")).frame(width: g.size.width*0.55*CGFloat(batteryPct), height: 5)
                    }.frame(width: g.size.width*0.55).position(x: g.size.width*0.36, y: 14)
                    Text("\(Int(batteryPct*100))%").font(.system(size: 8, weight: .medium)).foregroundColor(Color(hex: "22C55E")).position(x: g.size.width*0.76, y: 14)
                    ...
                    ZStack {
                        ...
                        Text("\(Int(e.speed)) km/h").font(.system(size: 13, weight: .bold, design: .rounded)).foregroundColor(Color(hex: "FF6B35"))
        ...
        let content = ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    Color(hex: "1A1230")
                    VStack(spacing: 3) {
                        Text(e.vehicleName).font(.system(size: 11, weight: .medium)).foregroundColor(Color(hex: "A78BFA"))
                ...
                ZStack {
                    ...
                    VStack(spacing: 0) {
                        Text("\(Int(batteryPct*100))%").font(.system(size: 9, weight: .bold)).foregroundColor(Color(hex: "A78BFA"))
        ...
        let content = ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    Color.clear
                    VStack(spacing: 2) {
                        Text("\(Int(e.speed))").font(.system(size: 36, weight: .black, design: .rounded)).foregroundColor(Color(hex: "FF6B35"))
                ...
                GeometryReader { g in
                    ZStack(alignment: .leading) {
                        ...
                        Rectangle().fill(Color(hex: "00F0FF")).frame(width: g.size.width*CGFloat(batteryPct))
                    }
                }.frame(height: 18).overlay(Text("\(Int(batteryPct*100))% battery").font(.system(size: 9, weight: .bold)).foregroundColor(.black))
            }
        }.clipShape(RoundedRectangle(cornerRadius: 22, style: .continuous))
    ...
    var body: some View {
        ...
        let content = ZStack {
            ...
            VStack(spacing: 0) {
                Spacer()
                // Speed
                Text("\(Int(e.speed))").font(.system(size: 50, weight: .black, design: .rounded)).foregroundColor(Color(hex: "FF2D6B"))
                ...
                GeometryReader { g in
                    ZStack(alignment: .leading) {
                        ...
                        Rectangle().fill(Color(hex: "FF2D6B")).frame(width: g.size.width*CGFloat(speedFrac), height: 2)
                    }.offset(y: 1)
                ...
                Text("\(Int(batteryPct*100))% battery").font(.system(size: 10)).foregroundColor((Color(hex: "00E5FF")).opacity(0.6)).padding(.top, 4)
                
                // Battery bar
                GeometryReader { g in
                    ZStack(alignment: .leading) {
                        Rectangle().fill((Color(hex: "00E5FF")).opacity(0.15)).frame(height: 2)
                        Rectangle().fill(Color(hex: "00E5FF")).frame(width: g.size.width*CGFloat(batteryPct), height: 2)
                    }.offset(y: 1)
                ...
                ZStack {
                    Color(hex: "111111")
                    Text("\(Int(e.speed)) km/h · speed").font(.system(size: 9, weight: .bold)).foregroundColor(Color(hex: "FF2D6B"))
                        ...
                        VStack(spacing: 3) {
                            GeometryReader { bg in
                                ZStack(alignment: .leading) {
                                    ...
                                    RoundedRectangle(cornerRadius: 3).fill(Color(hex: "22C55E")).frame(width: bg.size.width*CGFloat(batteryPct), height: 6)
                                }
                            }.frame(height: 6).padding(.horizontal, 10)
                            Text("\(Int(batteryPct*100))% batt · " + e.date.dateString)
...
struct RadarView: View {
    ...
    var body: some View {
        ...
        let batteryPct = max(0.05, min(1.0, e.batteryLevel))
        let spd = max(0.01, min(1.0, e.speed / 200.0))
        ...
        let content = ZStack {
            ...
            GeometryReader { g in
                ...
                ZStack {
                    ...
                    // Speed arc
                    Path { p in p.addArc(center: CGPoint(x: cx, y: cy), radius: R, startAngle: .radians(startA), endAngle: .radians(startA+spd*sweep), clockwise: false) }
                        .stroke(Color(hex: "00E5FF"), style: StrokeStyle(lineWidth: sw, lineCap: .round))
                    
                    // Tick marks
                    ForEach(0...10, id: \.self) { i in
                        let a = startA + (Double(i)/10)*sweep
                        let lit = Double(i)/10 <= spd
                        Path { p in
                    ...
                    // Speed number
                    Text("\(Int(e.speed))").font(.system(size: s*0.19, weight: .black, design: .rounded)).foregroundColor(Color(hex: "00E5FF")).position(x: cx, y: cy-s*0.04)
                    ...
                    ZStack(alignment: .leading) {
                        ...
                        RoundedRectangle(cornerRadius: 4).fill(Color(hex: "22C55E")).frame(width: (g.size.width-80)*CGFloat(batteryPct), height: 8)
                    }.frame(width: g.size.width-80).position(x: cx, y: g.size.height-18)
                    Text("\(Int(batteryPct*100))%").font(.system(size: 8)).foregroundColor(Color(hex: "22C55E")).position(x: cx, y: g.size.height-8)
                }
            }
        }.clipShape(RoundedRectangle(cornerRadius: 22, style: .continuous))
            ...
            VStack(spacing: 6) {
                ...
                HStack(spacing: 6) {
                    ...
                    ZStack {
                        ...
                        VStack(spacing: 1) {
                            Text("\(Int(e.speed))").font(.system(size: 24, weight: .black, design: .rounded)).foregroundColor(Color(hex: "FF6B35"))
                    ...
                    ZStack {
                        ...
                        ZStack {
                            ...
                            Text("\(Int(batteryPct*100))%").font(.system(size: 11, weight: .bold)).foregroundColor(Color(hex: "22C55E"))
                        }
                    }
                }.frame(maxWidth: .infinity, maxHeight: .infinity)
            }.padding(8)
        ...
        let content = ZStack {
            ...
            GeometryReader { g in
                ...
                let orbits: [(Double, String, Double, Double)] = [
                    ...
                    (batteryPct, "06B6D4", 55, 5),
                    (speedFrac, "F59E0B", 44, 5),
                ...
                ZStack {
                    ...
                    Text("\(Int(e.speed))km").font(.system(size: s*0.055)).foregroundColor((Color(hex: "7C3AED")).opacity(0.6)).position(x: cx, y: cy+s*0.07)
                }
            }
        }.clipShape(RoundedRectangle(cornerRadius: 22, style: .continuous))
